...
'''
from Configrations import TopConfig
...
# simutimes_range: [min_simutimes, max_simutimes]
    ...
    ## build denoising network for two distinct CNNs
    config = TopConfig()  # Load configurations
    cnn_model1 = ConvNet.ConvNet(config.CNN1_config, None, 0, 'CNN1')
    cnn_model2 = ConvNet.ConvNet(config.CNN2_config, None, 1, 'CNN2')
    denoise_net_in1, denoise_net_out1 = cnn_model1.build_network()
    denoise_net_in2, denoise_net_out2 = cnn_model2.build_network()
    ...
    # restore denoising network for both CNNs
    cnn_model1.restore_network_with_model_id(sess, config.CNN1_config.total_layers, model_id)
    cnn_model2.restore_network_with_model_id(sess, config.CNN2_config.total_layers, model_id)
    ...
    for SNR in SNRset:
        ...
        for ik in range(0, max_batches):
            ...
            for iter in range(0, denoising_net_num+1):
                ...
                if iter < denoising_net_num:
                    # Determine residual noise power or empirical distribution
                    # Adjust here to use cnn_model1 or cnn_model2 as needed
                    cnn_model = cnn_model1 if iter % 2 == 0 else cnn_model2
                    denoise_net_in = denoise_net_in1 if iter % 2 == 0 else denoise_net_in2
                    denoise_net_out = denoise_net_out1 if iter % 2 == 0 else denoise_net_out2

                    if top_config.update_llr_with_epdf:
                        prob = cnn_model.get_res_noise_pdf(model_id).get(np.float32(SNR))
                    else:
                        res_noise_power = cnn_model.get_res_noise_power(model_id, SNRset).get(np.float32(SNR))
                    
                    LLR = denoising_and_calc_LLR_awgn(res_noise_power, y_receive, u_BP_decoded, denoise_net_in, denoise_net_out, sess)
