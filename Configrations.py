...
class TopConfig:
    def __init__(self):
        ...
        self.corr_para = 0.2  # correlation parameters of the colored noise
        self.corr_para_simu = 0.9  # correlation parameters for simulation. this should be equal to corr_para. If not, it is used to test the model robustness.
        ...
        self.SNR_set_gen_data = np.array([-0.5], dtype=np.float32)

        # Simulation
        #self.eval_SNRs = np.array([0,0.5,1,1.5,2,2.5,3], np.float32)
        self.eval_SNRs = np.array([-0.5], np.float32)
